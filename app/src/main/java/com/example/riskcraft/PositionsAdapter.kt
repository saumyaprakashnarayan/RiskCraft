...
import com.example.riskcraft.realtime.MarketWebSocket
...
class PositionsAdapter(
    ...
    override fun onBindViewHolder(holder: PositionViewHolder, position: Int) {
        ...
        holder.details.text =
            "Qty: ${item.quantity} • Avg: ₹${item.avgPrice}"

        // 🔥 LIVE PRICE LOGIC
        val livePrice = MarketWebSocket.livePrices[item.symbol] ?: item.ltp
        val pnlValue = (livePrice - item.avgPrice) * item.quantity

        holder.pnl.text = String.format("₹%.2f", pnlValue)

        holder.pnl.setTextColor(
            if (pnlValue >= 0) 0xFF4CAF50.toInt()
